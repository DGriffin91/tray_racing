...
pub fn tune(init_options: Options, mut event_loop: winit::event_loop::EventLoop<()>) {
    ...
    let splits = [false, true];
    let search_distances = [1, 6, 14, 32];
    let sort_precisions = [64, 128];
    let reinsertion_batch_ratios = [0.5, 0.2, 0.1, 0.01, 0.005, 0.001, 0.0];
    let search_depth_thresholds = [0, 1, 2, 3, 4, 5];
            ...
            for sort_precision in sort_precisions {
                for reinsertion_batch_ratio in reinsertion_batch_ratios {
                    for search_depth_threshold in search_depth_thresholds {
                        ...
                        best_avg_traversal_time = best_avg_traversal_time.min(avg_traversal_time);
                        ...
                        let avg_permutation_duration = elapsed_time / (results.len() as f32);
                        let expected_remaining_duration =
                            (permutations - results.len()) as f32 * avg_permutation_duration;
    ...
    save_results(&blas_filtered_results);
    save_results(&tlas_filtered_results);

    fn save_results(tlas_filtered_results: &[TuningSet]) {
        match save_tuning_results_to_csv(&tlas_filtered_results, "tlas_filtered_results") {
