    ...
    PackedBlBvhNode node)
{
    ...
    float3 e = float3(asfloat(e_x << 23),
                      asfloat(e_y << 23),
                      asfloat(e_z << 23));

    float3 adjusted_ray_dir_inv[4] = { e / ray[0].direction, e / ray[1].direction, e / ray[2].direction, e / ray[3].direction };
    const float3 adjusted_ray_origins[4] = {
        (p - ray[0].origin) / ray[0].direction,
        (p - ray[1].origin) / ray[1].direction,
        (p - ray[2].origin) / ray[2].direction,
        (p - ray[3].origin) / ray[3].direction
    };
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        for (int ray_n = 0; ray_n < 4; ray_n++)
        {
            // TODO is there a good way to skip testing other rays if one has already intersected? Maybe a seperate binary hitmask?

            const uint x_min = ray[ray_n].direction.x < 0.0 ? q_hi_x : q_lo_x;
            const uint x_max = ray[ray_n].direction.x < 0.0 ? q_lo_x : q_hi_x;

            const uint y_min = ray[ray_n].direction.y < 0.0 ? q_hi_y : q_lo_y;
            const uint y_max = ray[ray_n].direction.y < 0.0 ? q_lo_y : q_hi_y;

            const uint z_min = ray[ray_n].direction.z < 0.0 ? q_hi_z : q_lo_z;
            const uint z_max = ray[ray_n].direction.z < 0.0 ? q_lo_z : q_hi_z;
            ...
            for (int j = 0; j < 4; j++)
            {
                ...
                const float tmax = min4(tmax3.x, tmax3.y, tmax3.z, ray_hit[i].t);
...
void traverse_bvh(Ray ray[4], inout RtOutput hit[4])
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ray[i].direction = select(ray[i].direction == 0.0, F32_EPSILON.xxx, ray[i].direction);
    }

    BvhStack stack = (BvhStack)0;
    uint2 current_group = uint2(0, 0);

    CwBvhRayHit ray_hit[4] = { (CwBvhRayHit)0, (CwBvhRayHit)0, (CwBvhRayHit)0, (CwBvhRayHit)0 };

    const uint oct_inv4 = ray_get_octant_inv4(ray[0].direction);
    ...
    while (true)
    {
...
#endif
            uint hitmask = cwbvh_node_intersect(ray, oct_inv4, ray_hit, node);
        ...
        while (triangle_group.y != 0)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                if (intersect_ray_tri(ray[i], tri, ray_hit[i].t, barycentric))
                {
                    ray_hit[i].triangle_id = global_triangle_index;
                }
            }
        }
    ...
    for (int i = 0; i < 4; i++)
    {
        if (ray_hit[i].triangle_id != -1 && ray_hit[i].t < hit[i].t)
        {
            hit[i].t = ray_hit[i].t;
            hit[i].primitive_id = ray_hit[i].triangle_id;
        }
    }
}

void intersects_bl_bvh(Ray ray[4])
{
    RtOutput tmp[4];
    tmp[0].t = 1e10;
    tmp[1].t = 1e10;
